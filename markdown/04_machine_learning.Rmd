...
Next, we show the 30 most frequenz words, both for tweets related to ADRs and tweets not related to ADRs.
...
Next, we take the `sentiments` data from the `tidytext` package to do sentiment analysis. We see quite a big difference in sentiments between tweets corresponding to adverse events and those not corresponding to adverse events:
...
One possibility to add additional features to the models, would be to train a second model and use the result of this model as input to a **stacked model**. This ansatz was tried, but it did not lead to improvement in predictive performance.
...
The next step is to aggregate the predictions from different models into a final model. We chose just the models based on the bag of words features (adding the result from the second model above did not significantly improve the area under curve and thus was not included).
...
pred_new_tweets <- function() {
    ...
    par(mfrow = c(2, 2))
    preds_new %$% pROC::roc(cleaned_tweet$is_AE, MAXENTROPY_PROB) %>% plot
    preds_new %$% pROC::roc(cleaned_tweet$is_AE, GLMNET_PROB) %>% plot
    preds_new %$% pROC::roc(cleaned_tweet$is_AE, SVM_PROB) %>% plot
    preds_new %$% pROC::roc(cleaned_tweet$is_AE, TREE_PROB) %>% plot
